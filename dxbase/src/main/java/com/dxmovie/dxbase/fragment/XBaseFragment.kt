...
import com.dxmovie.dxbase.viewmodel.BaseViewModel.UILiveData.TYPE
...
open class XBaseFragment : BaseViewModelFragment() {
    ...
    open fun initUI(viewModel: ViewModel) {
        if (viewModel is BaseViewModel) {
            ...
            viewModel.ui?.statusLiveData?.observe(this, Observer {
                when (it.what) {
                    BaseViewModel.UILiveData.TYPE.SHOWLOADINGDIALOG -> showLoadingDialog()?.setDissmissType(it.arg1)
     ...
     */
    open fun getStatusViewLayout(): StatusViewLayout? {
